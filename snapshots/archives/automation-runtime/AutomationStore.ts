...
class AutomationStoreService {
  ...
  public matchVoiceTriggeredAutomation(text: string, ownerId: string | null): AutomationRecord | null {
    ...
    for (const automation of this.automations.values()) {
      ...
      const phrases = [automation.trigger.phrase, ...automation.trigger.aliases].map((entry) => entry.trim().toLowerCase()).filter(Boolean);
      const matched = phrases.find((entry) => entry === normalized || (automation.trigger.sensitivity !== 'low' && normalized.includes(entry)));
      if (matched) {
  ...
  private dispatchAutomation(automation: AutomationRecord, reason: string): boolean {
    ...
    if (automation.activeTaskIds.some((taskId) => {
      ...
      return Boolean(task) && ['pending', 'queued', 'running', 'waiting'].includes(task.status);
    })) {
      automation.continuity.metrics.skippedExecutions += 1;
  ...
  private seedDefaultsForCurrentUser(): void {
    ...
    if (this.seedState.has(ownerId)) {
      logger.info('AUTOMATION_PERSISTENCE', `seed_skipped ownerId=${ownerId} reason=owner_initialized`);
      return;
    }

    const existing = Array.from(this.automations.values()).filter((automation) => automation.ownerId === ownerId);
    ...
    this.createAutomation({
      ...
      description: 'Open Spotify and say Bom dia every weekday at 07:30.',
      type: 'TIME_BASED',
      enabled: true,
      trigger: {
        type: 'TIME_BASED',
      ...
      conditions: [],
      actions: [
        { id: 'morning_spotify', kind: 'LOCAL_COMMAND', label: 'Open Spotify', command: 'open_app', appTarget: 'spotify' },
        { id: 'morning_greeting', kind: 'VOICE_ACTION', label: 'Say Bom Dia', speechText: 'Bom dia', delayMs: 1200 },
      ],
      schedule: { retries: 1, cooldownMs: 0, priority: 1, requireConfirmation: false, repeatEnabled: true, activeWindowStart: null, activeWindowEnd: null, weeklyExecutionLimit: null },
    });

    this.createAutomation({
      ownerId,
      name: 'Recovery Watch',
      description: 'Reconnect runtime and refresh hydration when the socket disconnects.',
      type: 'SYSTEM_TRIGGERED',
      enabled: true,
      trigger: { type: 'SYSTEM_TRIGGERED', event: 'socket_disconnected' },
      conditions: [],
      actions: [
        { id: 'reconnect_runtime', kind: 'SYSTEM_ACTION', label: 'Reconnect Runtime', action: 'runtime_socket_reconnect' },
        { id: 'refresh_hydration', kind: 'SYSTEM_ACTION', label: 'Refresh Hydration', action: 'runtime_hydration_revalidate', delayMs: 1200 },
      ],
      schedule: { retries: 2, cooldownMs: 30_000, priority: 2, requireConfirmation: false, repeatEnabled: true, activeWindowStart: null, activeWindowEnd: null, weeklyExecutionLimit: null },
    });

    this.createAutomation({
      ownerId,
      name: 'Focus Mode',
      description: 'Enable focus mode with a custom voice phrase.',
      type: 'VOICE_TRIGGERED',
      enabled: true,
      trigger: { type: 'VOICE_TRIGGERED', phrase: 'focus mode', aliases: ['start focus mode'], sensitivity: 'medium' },
      conditions: [],
      actions: [
        { id: 'focus_activate', kind: 'LOCAL_COMMAND', label: 'Activate Focus Mode', command: 'focus_mode' },
      ],
      schedule: { retries: 1, cooldownMs: 10_000, priority: 1, requireConfirmation: false, repeatEnabled: true, activeWindowStart: null, activeWindowEnd: null, weeklyExecutionLimit: null },
    });

    this.createAutomation({
      ownerId,
      name: 'Gaming Mode',
      description: 'Manual routine for Discord and performance mode.',
      type: 'MANUAL',
      enabled: true,
      trigger: { type: 'MANUAL' },
      conditions: [],
      actions: [
        { id: 'gaming_discord', kind: 'LOCAL_COMMAND', label: 'Open Discord', command: 'open_app', appTarget: 'discord' },
        { id: 'gaming_performance', kind: 'LOCAL_COMMAND', label: 'Performance Mode', command: 'performance_mode', delayMs: 900 },
      ],
      schedule: { retries: 1, cooldownMs: 0, priority: 1, requireConfirmation: false, repeatEnabled: false, activeWindowStart: null, activeWindowEnd: null, weeklyExecutionLimit: null },
    });

    this.markOwnerInitialized(ownerId);
    logger.info('AUTOMATION_HYDRATION', `seeded_defaults ownerId=${ownerId}`);
  }
