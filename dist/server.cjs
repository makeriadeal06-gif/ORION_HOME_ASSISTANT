...
var TriggerCMDService = class _TriggerCMDService {
  ...
  async executeRemoteWithToken(endpoint, token, device) {
    ...
    console.log("[TRIGGER_HTTP_RESPONSE]", {
      ...
      ok: response.ok,
      body_preview: String(responseText).substring(0, 2e3)
    });
    ...
    if (Array.isArray(root.computers)) {
      ...
      for (const comp of root.computers) {
        ...
        if (Array.isArray(compObj.commands)) {
          const compId = this.toStringValue(compObj.id) || this.toStringValue(compObj._id) || this.toStringValue(compObj.computer_id) || this.toStringValue(compObj.computerId) || void 0;
  ...
  normalizeDevice(raw) {
    ...
    const computerIdCandidate = this.toStringValue(device["computer_id"]) || this.toStringValue(device["computerId"]) || this.toStringValue(device["computerID"]) || this.toStringValue(computer.id) || this.toStringValue(computer._id) || null;
    ...
    return {
      ...
      app: app2,
      category,
      computerId: computerIdCandidate || void 0
    };
  }
  // ──────────────── Helpers ────────────────
  sanitizeToken(value) {
