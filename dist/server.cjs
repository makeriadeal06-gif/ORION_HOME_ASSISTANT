...
async function startServer() {
  ...
  const activeVoiceRequests = /* @__PURE__ */ new Set();
  ...
  app.post("/api/voice/tts", async (req, res) => {
    ...
    try {
      ...
      const body = err instanceof ElevenLabsServiceError ? err.responseBody : void 0;
  ...
  io.on("connection", (socket) => {
    ...
    socket.emit("mqtt:status", {
      connected: backendMqttManager.connected,
      timestamp: Date.now()
    });
